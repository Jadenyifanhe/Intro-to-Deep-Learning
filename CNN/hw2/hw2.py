...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for i in range(self.nlayers):
            self.convolutional_layers.append(
                       Conv1d(in_channels=self.num_input_channels,
                       out_channels=num_channels[i],
                       kernel_size=kernel_sizes[i],
                       stride=strides[i],
                       weight_init_fn=conv_weight_init_fn,
                       bias_init_fn=bias_init_fn))
            self.num_input_channels = num_channels[i]
            self.output_width = ((self.input_width - kernel_sizes[i]) // strides[i]) + 1
            self.input_width = self.output_width
        self.flatten = Flatten()
        self.linear_layer = Linear(in_features=self.num_input_channels * self.output_width,
    ...
    def forward(self, A):
        ...
        A_flat = self.flatten.forward(A)
        A_out = self.linear_layer.forward(A_flat)
    ...
    def backward(self, labels):
        ...
        for i in range(self.nlayers-1, -1, -1):
            dLdZ = grad * self.activations[i].backward()
            grad = self.convolutional_layers[i].backward(dLdZ)
