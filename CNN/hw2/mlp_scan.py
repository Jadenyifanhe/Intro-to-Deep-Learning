...
class CNN_SimpleScanningMLP():
    def __init__(self):
        ...
        # ...
        # <---------------------
        self.conv1 = Conv1d(in_channels=8 * 24, out_channels=8, kernel_size=8, stride=4)
    ...
    def init_weights(self, weights):
        ...
        w1_transpose = np.reshape(np.transpose(w1), (8, 8, 24))
        w2_transpose = np.reshape(np.transpose(w2), (16, 1, 8))
        w3_transpose = np.reshape(np.transpose(w3), (4, 1, 16))

        self.conv1.conv1d_stride1.W = np.transpose(w1_transpose, (0, 2, 1))
        self.conv2.conv1d_stride1.W = np.transpose(w2_transpose, (0, 2, 1))
        self.conv3.conv1d_stride1.W = np.transpose(w3_transpose, (0, 2, 1))
...
class CNN_DistributedScanningMLP():
    ...
    def init_weights(self, weights):
        ...
        w1_transpose = np.reshape(np.transpose(w1[:2 * 24, :2]), (2, 2, 24))
        w2_transpose = np.reshape(np.transpose(w2[:2 * 2, :8]), (8, 2, 2))
        w3_transpose = np.reshape(np.transpose(w3[:2 * 8, :4]), (4, 2, 8))
