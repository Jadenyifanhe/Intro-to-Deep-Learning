...
class BatchNorm1d:
    ...
    def backward(self, dLdBZ):
        ...
        dLdV = (-1 / 2) * np.sum(np.multiply(np.multiply(dLdNZ, (self.Z - self.M)), (self.V + self.eps) ** (-3 / 2)),
                                 axis=0)
        dLdM = (- np.sum(dLdNZ * ((self.V + self.eps)) ** (-1 / 2), axis=0)) - (
                    (2 / self.N) * dLdV * np.sum(self.Z - self.M))
